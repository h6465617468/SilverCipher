...
{
public static function Eraser0($filename){$methods=array("NIST SP-800-88 Rev. 1","Gutmann","DoD 5220.22-M","NSA 130-2","ATA Secure Erase","Cryptographic Erase","Purge","Clear");$patterns=array("\x00\xFF","\xFF\x00","\x55\xAA","\xAA\x55","\x92\x49","\x49\x92","\x24\x92","\x92\x24","\x6D\xB6","\xB6\x6D","\xDB\xDB","\x6D\xB6","\xFF\xFF","\x00\x00","\x11\x11","\x22\x22","\x33\x33","\x44\x44","\x55\x55","\x66\x66","\x77\x77","\x88\x88","\x99\x99","\xAA\xAA","\xBB\xBB","\xCC\xCC","\xDD\xDD","\xEE\xEE","\xFF\xFF","\x00\x00","\x00\x00","\xFF\xFF","\xAA\xAA","\x55\x55","\x00\x00","\xFF\xFF","\x00\x00","\xFF\xFF","\x55\x55","\xAA\xAA","\xFF\xFF","\x00\x00","\xAA\xAA","\x55\x55","\xFF\xFF","\x00\x00","\x55\x55","\xAA\xAA");$pattern_count=count($patterns);for($k=0;$k<5;$k++){foreach($methods as $method){$file=fopen($filename,"r+");$size=filesize($filename);$pattern='';if($method=="NIST SP-800-88 Rev. 1"){$pattern=str_repeat("\x00\xFF\x00\xFF",(int)(($size+3)/4));}elseif($method=="Gutmann"){$pattern='';for($i=0;$i<35;$i++){$pattern.=chr($i%256);}$pattern=str_repeat($pattern,(int)(($size+34)/35));}elseif($method=="DoD 5220.22-M"){$pattern=str_repeat("\x55\x55\x55\x55\x55",(int)(($size+4)/5));}elseif($method=="NSA 130-2"){$pattern=str_repeat("\x00",$size);}elseif($method=="ATA Secure Erase"){$pattern=str_repeat("\x00\x00\x00\x00\x00\x00\x00\x00",(int)(($size+7)/8));}elseif($method=="Cryptographic Erase"){$pattern=openssl_random_pseudo_bytes($size);}elseif($method=="Purge"){$pattern=str_repeat("\x00",$size);}elseif($method=="Clear"){$pattern=str_repeat("\xFF",$size);}for($i=0;$i<$size;$i+=strlen($string)){fseek($file,$i);$pattern_index=($i/strlen($string))%$pattern_count;$string_index=$i%strlen($string);$byte=$string[$string_index]^$patterns[$pattern_index][$string_index%2];fwrite($file,$byte,1);}fclose($file);}}}
