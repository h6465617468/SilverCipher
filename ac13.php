...
How to use?
<?php
$text = "Best encryption algorithm";
$key = "Antares";
# Use (Encrypt): 
$encrypted_text = Antares_Crypt::Encrypt($text,$key);
# Use (Decrypt): 
$decrypted_text = Antares_Crypt::Decrypt($encrypted_text,$key);
...
Class Antares_Crypt
{
    Public Static Function Encrypt($string,$key)
    {
        ...
        $st=self::settingsgenerator(self::Hex_Dont_Count(hash("whirlpool",$key.$salt)));
        ...
        foreach ($data_x_all as $str_x)
        {
            ...
            if($counter!=$block_counter)
            {
                ...
                $salt=self::XOREncrypt($key,hex2bin(self::Hex_Dont_Count(hash("sha512",$key.$salt))));
                ...
                $st=self::settingsgenerator2(self::Hex_Dont_Count(hash("snefru",$key.$salt)));
            }
            $data=$data.$veri[0];
            $counter++;
            $counter_st++;
    ...
    Private Static Function Enc($string,$key,$st)
    {
        ...
        $data=self::E_death_round($data,$st,$keyx);
        $data=strrev($data);
    ...
    Private Static Function E_death_round($data,$settings,$keyx)
    {
        ...
        for ($i = 0; $i <= strlen(trim($settings))-1; $i++)
        {
            ...
            if($a=="0")
            {
                $data=strtr($data,self::$base64_characters,self::$rot13_2);
                $data=strtr($data,self::$base64_characters,self::$hashb);
                $data=self::Hex_Encrypt_Key($data,$keyx);
                $data=self::encrypt_key($keyx,$data);
                $data=strtr($data,self::$base64_characters,self::$hash2);
            }
            if($a=="1")
            {
                $data=strtr($data,self::$base64_characters,self::$hashf);
                $data=self::encrypt_key($keyx,$data);
                $data=strtr($data,self::$base64_characters,self::$hash6);
                $data=self::Hex_Encrypt_Key($data,$keyx);
                $data=strtr($data,self::$base64_characters,self::$hash1);
            }
            if($a=="2")
            {
                $data=strtr($data,self::$base64_characters,self::$hashd);
                $data=strtr($data,self::$base64_characters,self::$rot13_1);
                $data=strtr($data,self::$base64_characters,self::$hash0);
                $data=self::encrypt_key($keyx,$data);
                $data=strtr($data,self::$base64_characters,self::$hash3);
            }
            if($a=="3")
            {
                $data=self::Hex_Encrypt_Key($data,$keyx);
                $data=strtr($data,self::$base64_characters,self::$hash4);
            ...
            if($a=="4")
            {
                $data=strtr($data,self::$base64_characters,self::$rot13_3);
                $data=strtr($data,self::$base64_characters,self::$hash8);
                $data=strtr($data,self::$base64_characters,self::$hash9);
                $data=strtr($data,self::$base64_characters,self::$hasha);
                $data=self::encrypt_key($keyx,$data);
            }
            if($a=="5")
            {
                $data=self::Hex_Encrypt_Key($data,$keyx);
                $data=strtr($data,self::$base64_characters,self::$rot13_4);
                $data=strrev($data);
                $data=self::encrypt_key($keyx,$data);
                $data=self::bk_kb($data);
            }
            if($a=="6")
            {
                $data=strtr($data,self::$base64_characters,self::$hash5);
                $data=self::bk_kb($data);
                $data=strrev($data);
                $data=strtr($data,self::$base64_characters,self::$hashe);
                $data=strtr($data,self::$base64_characters,self::$rot13_4);
            }
            if($a=="7")
            {
                $data=self::Hex_Encrypt_Key($data,$keyx);
                $data=strtr($data,self::$base64_characters,self::$hash7);
                $data=strtr($data,self::$base64_characters,self::$hashc);
                $data=strrev($data);
                $data=self::encrypt_key($keyx,$data);
            }
        }
    ...
    Private Static Function D_death_round($data,$settings,$keyx)
    {
        $data=self::decrypt_key($keyx,$data);
        ...
        for ($i = 0; $i <= strlen(strrev($settings))-1; $i++)
        {
            ...
            if($a=="0")
            {
                $data=strtr($data,self::$hash2,self::$base64_characters);
                $data=self::decrypt_key($keyx,$data);
                $data=self::Hex_Decrypt_Key($data,$keyx);
                $data=strtr($data,self::$hashb,self::$base64_characters);
                $data=strtr($data,self::$rot13_2,self::$base64_characters);
            }
            if($a=="1")
            {
                $data=strtr($data,self::$hash1,self::$base64_characters);
                $data=self::Hex_Decrypt_Key($data,$keyx);
                $data=strtr($data,self::$hash6,self::$base64_characters);
                $data=self::decrypt_key($keyx,$data);
                $data=strtr($data,self::$hashf,self::$base64_characters);
            }
            if($a=="2")
            {
                $data=strtr($data,self::$hash3,self::$base64_characters);
                $data=self::decrypt_key($keyx,$data);
                $data=strtr($data,self::$hash0,self::$base64_characters);
                $data=strtr($data,self::$rot13_1,self::$base64_characters);
                $data=strtr($data,self::$hashd,self::$base64_characters);
            }
            if($a=="3")
            {
                $data=strrev($data);
                $data=self::Hex_Decrypt_Key($data,$keyx);
                $data=self::bk_kb($data);
                $data=strtr($data,self::$hash4,self::$base64_characters);
                $data=self::Hex_Decrypt_Key($data,$keyx);
            }
            if($a=="4")
            {
                $data=self::decrypt_key($keyx,$data);
                $data=strtr($data,self::$hasha,self::$base64_characters);
                $data=strtr($data,self::$hash9,self::$base64_characters);
                $data=strtr($data,self::$hash8,self::$base64_characters);
                $data=strtr($data,self::$rot13_3,self::$base64_characters);
            }
            if($a=="5")
            {
                $data=self::bk_kb($data);
                $data=self::decrypt_key($keyx,$data);
                $data=strrev($data);
                $data=strtr($data,self::$rot13_4,self::$base64_characters);
                $data=self::Hex_Decrypt_Key($data,$keyx);
            }
            if($a=="6")
            {
                $data=strtr($data,self::$rot13_4,self::$base64_characters);
                $data=strtr($data,self::$hashe,self::$base64_characters);
                $data=strrev($data);
                $data=self::bk_kb($data);
                $data=strtr($data,self::$hash5,self::$base64_characters);
            }
            if($a=="7")
            {
                $data=self::decrypt_key($keyx,$data);
                $data=strrev($data);
                $data=strtr($data,self::$hashc,self::$base64_characters);
                $data=strtr($data,self::$hash7,self::$base64_characters);
                $data=self::Hex_Decrypt_Key($data,$keyx);
            }
        }
        $data=strrev($data);
