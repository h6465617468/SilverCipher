...
{
    static function Encrypt($a = null, $b = null)
    {
        $b = base64_decode(trim(self::$salt)) . hex2bin(Antares_Crypt_v1_5::Hex_Dont_Count(self::Antares_Crypt_HASH(hex2bin(hash("whirlpool", $b)), 1024)));
        $a = self::Kiyim($a, "e", $b);
        $f = str_split($a);
        $a = null;
        $d = strtr(md5($b), self::$st_hex_chars, self::$st_hex_chang);
        $c = 0;
        foreach ($f as $e) {
            if ($c == 32) {
                $d = strtr(md5($b), self::$st_hex_chars, self::$st_hex_chang);
            ...
            if ($d[$c] == 0) {
                $a = $a . str_replace("=", "", Antares_Crypt_v1_1::Encrypt($e, $b, "no"));
            } else {
                if ($d[$c] == 1) {
                    $a = $a . str_replace("=", "", Antares_Crypt_v1_2::Encrypt($e, $b, "no"));
                } else {
                    if ($d[$c] == 2) {
                        $a = $a . str_replace("=", "", Antares_Crypt_v1_3_no_compress::Encrypt($e, $b));
                    } else {
                        if ($d[$c] == 3) {
                            $a = $a . str_replace("=", "", Antares_Crypt_v1_4::Encrypt($e, $b));
                        } else {
                            if ($d[$c] == 4) {
                                $a = $a . str_replace("=", "", Antares_Crypt_v1_5::Encrypt($e, $b));
                            } else {
                            }
                        }
                    }
    ...
    static function Decrypt($a = null, $b = null)
    {
        ...
        foreach ($g as $e) {
            ...
            if ($d[$c] == 0) {
                $a = $a . Antares_Crypt_v1_1::Decrypt($e, $b, "no");
            } else {
                if ($d[$c] == 1) {
                    $a = $a . Antares_Crypt_v1_2::Decrypt($e, $b, "no");
                } else {
                    if ($d[$c] == 2) {
                        $a = $a . Antares_Crypt_v1_3_no_compress::Decrypt($e, $b);
                    } else {
                        if ($d[$c] == 3) {
                            $a = $a . Antares_Crypt_v1_4::Decrypt($e, $b);
                        } else {
                            if ($d[$c] == 4) {
                                $a = $a . Antares_Crypt_v1_5::Decrypt($e, $b);
                            } else {
                            }
                        }
                    }
...
class Antares_Crypt_v1_5
{
    ...
    static function Enc($f = null, $b = null)
    {
        $g = $b;
        ...
        foreach ($j as $k) {
            ...
            if (strlen($f) > pow(2, 7)) {
                ...
                $e++;
                $b = hex2bin(self::Hex_Dont_Count(hash("sha512", self::XOREncrypt(strrev(self::XOREncrypt($b . chr(0xb5) . chr(0xb9), $c)), $a[1]) . $c . $a[1])));
            }
            $d = $d . $a[0];
            $h = $h . $a[1];
        }
    ...
    static function Dec($f, $c)
    {
        ...
        foreach ($j as $k) {
            ...
            if (strlen($f) > pow(2, 7)) {
                $d = false;
                $d = chr(self::$salt_2_dat[$e]);
                $i = self::numHash($c, 1);
                if ($i != 0) {
                    $d = self::XOREncrypt($d, chr(self::$salt_2_dat[$i]));
                }
                if ($e == 48) {
                    $e = 0;
                }
                $e++;
                $c = hex2bin(self::Hex_Dont_Count(hash("sha512", self::XOREncrypt(strrev(self::XOREncrypt($c . chr(0xb5) . chr(0xb9), $d)), $b[1]) . $d . $b[1])));
            }
            $a = $a . $b[0];
            $h = $h . $b[1];
        }
