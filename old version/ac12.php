...
Class Antares_Crypt
{
    Public Static Function Encrypt($string,$key,$key_mode="no")
    {
    ...
    Public Static Function Decrypt($string,$key,$key_mode="no")
    {
    ...
    {
        $keyx=$key;
        $ttt=hex2bin(md5($keyx));
        $key=hash("sha512",$key.$ttt.chr(0x3f).chr(0x6c).chr(0x33));
        $keyz=hex2bin(hash("sha512",hex2bin($key)));
        $keyy=hex2bin(hash("sha512",hex2bin(self::E_hex_1($key))));
        $keyxor=hex2bin(hash("sha512",hex2bin(md5(self::XOREncrypt(hex2bin($key),$keyy)))));
        $key444=self::hashtoXcode($key);
        $aWqXCufHCxQWxICEJC=hex2bin(hash("sha512",self::XOREncrypt($keyz,$keyx)));
        $yTECx8wqcx90sjc87v=hex2bin(hash("sha512",hex2bin(hash("sha512",$keyy))));
        if($action=="e")
        {
            ...
            $hashed_key=bin2hex(openssl_random_pseudo_bytes(64));
            $sayac1=0;
            $harf=false;
            ...
            foreach($harfler as $harfx)
            {
                if($sayac1==128)
                {
                    $sayac1=0;
                    $hashed_key=bin2hex(openssl_random_pseudo_bytes(64));
                    $key=hash("sha512",hex2bin($key).$ttt.chr(0x33).chr(0x99).chr(0xb4));
                    $key444=self::hashtoXcode($key);
                }
                if($key444[$sayac1]==0)
                {
                    $harf=$harf.self::E_hex_1(self::E_hex_1($harfx)).$hashed_key[$sayac1];
                ...
                {
                    $harf=$harf.self::E_hex_1(self::E_hex_1($harfx)).self::hashtohash_1(self::hashtohash_1($hashed_key[$sayac1]));
                    $harf_keyx=$harf_keyx.self::hashtohash_1(self::hashtohash_1($hashed_key[$sayac1]));
                }
                else if($key444[$sayac1]==3)
                {
                    $harf=$harf.self::hashtohash_1($hashed_key[$sayac1]).self::E_hex_1($harfx);
            ...
            $bolum2=hex2bin($harf);
            $bolum2=self::XOREncrypt($bolum2,$aWqXCufHCxQWxICEJC);
            $bolum2=self::XOREncrypt($bolum2,$keyz);
            $bolum2=self::XOREncrypt($bolum2,$yTECx8wqcx90sjc87v);
        ...
        else if($action=="d")
        {
            ...
            $bolum1=self::XORDecrypt($bolum1,$yTECx8wqcx90sjc87v);
            $bolum1=self::XORDecrypt($bolum1,$keyz);
            $bolum1=self::XORDecrypt($bolum1,$aWqXCufHCxQWxICEJC);
            ...
            foreach($harfler as $harfx)
            {
                ...
                {
                    $harf_sc=self::D_hex_1(self::D_hex_1(substr($harfx,0,1)));
